...
Dgrid is a Multicore, Data-Aware, Embedded RISC-V AI Accelerator for Edge Inferencing running on low-end FPGA devices or custom ASIC.

Acceleration provided by dgrid can be up to 20-50x compared with a non-accelerated RISCV implementation
on many vision/AI tasks. dgrid performs also better when compared with a RISCV that is equipped with vector extension.
...
to executing TensorFlow AI models. This is one key difference of dgrid when compared with other accelerators
that tend to accelerate only a narrow range of applications only (for example convolution neural network only).

A new tensor programming paradigm is introduced to allow programmers to leverage the massive processing/data parallelism enabled by dgrid tensor processor.

![Dgrid Architecture](./Documentation/images/dgrid_arch.png)

# Features
## Hardware
Dgrid consists of the following functional units tied via an AXI Bus to a DgsRiscv CPU, a DRAM and other
...
[![dgrid demo video](Documentation/images/demo_video.jpg)](Documentation/images/demo_video.mp4)
...
3. [Programmers Guide](https://github.com/dgrid/dgrid/raw/master/Documentation/dgrid_programmer_guide.pdf)

4. [VisionAI Stack Programmers Guide](https://github.com/dgrid/dgrid/raw/master/Documentation/visionai_programmer_guide.pdf)
...
```
.
├── Documentation         Overview on HW/SW and programmer's guide for dgrid, pcore, visionai and tensor
├── HW                    Hardware
│   ├── examples          Reference Design: Integration of Dgsriscv, Dgrid, DDR3, VGA, Camera, LEDs & Buttons
│   ├── platform          Memory IP depenedencies for different FPGA synthesis (e.g. XIlinx, Altera) or ASIC
│   ├── simulation        RTL Simulation
│   └── src               RTL of Dgrid's top design, Scalar/Vector ALU, Dataplane, Pcore, SoC integration etc
...
│   └── dgrid_port      dgrid micropython port
├── README.md
├── SW                    Software
│   ├── apps              AI kernel libraries of canny edge detector, harris corner, neural nets, optical flow etc
│   ├── base              C runtime zero, Dgrid application libraries and other utilities
│   ├── compiler          Dgrid C-like DSL compiler that generates instructions for the tensor processor
│   ├── fs                File for data inference to be downloaded together with the build image
│   ├── linker.ld         linker script for Dgrid
│   ├── makefile          Main project makefile
│   ├── makefile.kernels  Kernel makefile
│   ├── makefile.sim      Makefile to test Kernels
│   ├── sim               C source to test kernels
│   └── src               SW Main (visionai and unit test entry points), SoC drivers and Dgrid's micropython API
│                         This is a good place to learn on how to use dgrid prebuilt vision and AI stack.
└── tools                 openocd and dgsriscv interface descriptions
```
...
Also, in `SW/apps`, many prebuilt acceleration functions are provided to provide programmers with a fast path to leverage dgrid acceleration. This folder is also a good place to learn on how to program your own custom acceleration functions.

# Build procedure

The build procedure produces 2 seperate images.

One image is a standalone executable where user applications are using dgrid using a native [C/C++ library interface] (https://github.com/dgrid/dgrid/raw/master/Documentation/visionai_programmer_guide.pdf)

The second image is a micropython port of dgrid. With this image, applications are using dgrid using a [Python programming interface](micropython/MicropythonUserGuide.md)
...
## Download dgrid
```
git clone https://github.com/dgrid/dgrid.git
```

## Build dgrid as standalone image
```
export PATH=/opt/riscv/bin:$PATH
cd dgrid
cd SW/compiler
...
## Build dgrid as micropython port
...
cp -avr <dgrid installation folder>/micropython/dgrid_port .
cd dgrid_port
export PATH=/opt/riscv/bin:$PATH
export DGRID=<dgrid installation folder>
make clean
make
```
...
If you are running dgrid's micropython image, then you need to connect to the serial port. Arty-A7 provides serial port connectivity via USB. Serial port flow control must be disabled.

```
sudo minicom -w -D /dev/ttyUSB1
```

Note: After the first time connecting to serial port, reset the board again (press button next to USB port and wait for led to turn green) since USB serial must be the first device to connect to USB before dgrid.
...
make
cp <dgrid installation folder>/tools/openocd/soc_init.cfg .
cp <dgrid installation folder>/tools/openocd/usb_connect.cfg .
cp <dgrid installation folder>/tools/openocd/xilinx-xc7.cfg .
cp <dgrid installation folder>/tools/openocd/jtagspi.cfg .
cp <dgrid installation folder>/tools/openocd/cpu0.yaml .
```
...
```
export PATH=/opt/riscv/bin:$PATH
cd <dgrid installation folder>/SW/src
riscv32-unknown-elf-gdb ../build/dgrid.elf
```
...
```
export PATH=/opt/riscv/bin:$PATH
cd <Micropython installation folder>/ports/dgrid_port
...
# How to port dgrid to other FPGA,ASIC and SOC 

Click [here](Documentation/PortProcedure.md) for procedure on how to port dgrid and its applications to other FPGA/ASIC and SOC.

# Run dgrid in simulation
...
Copy the generated image <dgrid>/SW/build/dgrid_sim.hex to folder where you run your simulator. 
