package dgsriscv.riscv

import spinal.core._
import spinal.lib._
import dgsriscv.ip.{DataCacheConfig, InstructionCacheConfig}
...
import dgsriscv.plugin._
import dgsriscv.{DgsRiscv, DgsRiscvConfig, plugin}
import dgsriscv.ip.fpu.FpuParameter
...
case class RiscvConfig(
                       ...
                       cpuPlugins              : ArrayBuffer[Plugin[DgsRiscv]]){
...
case class riscv(config : RiscvConfig) extends Component{
  ...
  val system = new ClockingArea(systemClockDomain) {
    ...
    //Instanciate the CPU
    val cpu = new DgsRiscv(
      config = DgsRiscvConfig(
