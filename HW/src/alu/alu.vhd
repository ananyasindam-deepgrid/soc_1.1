...
-- Additional pipeline stages for x1 and x2 to support max/min operations
signal x1_rr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);
signal x1_rrr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);
signal x1_rrrr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);

signal x2_rr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);
signal x2_rrr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);
signal x2_rrrr : STD_LOGIC_VECTOR(register_width_c-1 DOWNTO 0);
...
begin
   ...
   if(mu_opcode_rrrr=mu_opcode_conv_c) then
      ...
      -- NEW: MaxPool operation - select maximum
   elsif(mu_opcode_rrrr=mu_opcode_max_c) then
   -- y_shift_r contains (x1 - x2)
   -- If result is non-negative, x1 >= x2, else x2 > x1
   if y_shift_r(accumulator_width_c-1) = '0' then
      y3_r <= x1_rrrr;  -- x1 is the maximum
   else
      y3_r <= x2_rrrr;  -- x2 is the maximum
   end if;
   
   -- NEW: MinPool operation - select minimum
   elsif(mu_opcode_rrrr=mu_opcode_min_c) then
      -- y_shift_r contains (x1 - x2)
      -- If result is negative, x1 < x2, else x2 <= x1
   if y_shift_r(accumulator_width_c-1) = '1' then
      y3_r <= x1_rrrr;  -- x1 is the minimum
   else
      y3_r <= x2_rrrr;  -- x2 is the minimum
   end if;
   
   -- NEW: Element-wise multiply - output multiply result directly
   elsif(mu_opcode_rrrr=mu_opcode_mul_elem_c) then
...
begin
   ...
   if clock_in'event and clock_in='1' then
      -- Element-wise multiply: multiply x1 by x2 (no accumulation)
      if mu_opcode_in=mu_opcode_mul_elem_c then
         mul_x2_r <= x1_in;
         
      elsif mu_opcode_in=mu_opcode_mul_c then
         mul_x2_r <= x1_in;

      elsif mu_opcode_in(fm_oc_hi_c downto fm_oc_lo_c)=mu_opcode_fm_c(fm_oc_hi_c downto fm_oc_lo_c) then
         mul_x2_r <= x1_in;

      -- For max/min operations, don't multiply (will use subtraction)
      -- elsif mu_opcode_in=mu_opcode_max_c or mu_opcode_in=mu_opcode_min_c then
      --   mul_x2_r <= (others=>'0');
...
begin
    if reset_in = '0' then
        ...
        if clock_in'event and clock_in='1' then
            ...
            -- Extended pipeline for operands (needed for max/min operations)
            x1_rr <= x1_r;
            x1_rrr <= x1_rr;
            x1_rrrr <= x1_rrr;
            ...
            if (mu_opcode_r(fm_oc_hi_c downto fm_oc_lo_c)=mu_opcode_fm_c(fm_oc_hi_c downto fm_oc_lo_c)) then
               ...
               case mu_opcode_r is
                  ...
                  -- NEW: Element-wise multiply, max, min - no accumulation
                  when mu_opcode_mul_elem_c =>     -- |mu_opcode_max_c|mu_opcode_min_c
                     xreg_rr <= (others=>'0');
            ...
            if mu_opcode_r(fm_oc_hi_c downto fm_oc_lo_c)=mu_opcode_fm_c(fm_oc_hi_c downto fm_oc_lo_c) then
                ...
                case mu_opcode_r is
                     ...
                     -- NEW: Max/Min operations need subtraction to compare
                     when mu_opcode_max_c | mu_opcode_min_c =>
                        add_sub_r <= '0';  -- Force subtraction (x1 - x2)
