...
architecture Behavioral of UART is
   ...
   constant FIFO_DEPTH:integer:=FIFO_DEPTH_G;
...
match_read <= apb_penable when (apb_paddr(apb_addr_len_c-1 downto 0)=std_logic_vector(to_unsigned(APB_REG_READ,apb_addr_len_c))) else '0';

match_read_avail <= apb_penable when (apb_paddr(apb_addr_len_c-1 downto 0)=std_logic_vector(to_unsigned(APB_REG_READ_AVAIL,apb_addr_len_c))) else '0';

match_write <= apb_penable when (apb_paddr(apb_addr_len_c-1 downto 0)=std_logic_vector(to_unsigned(APB_REG_WRITE,apb_addr_len_c))) else '0';

match_write_avail <= apb_penable when (apb_paddr(apb_addr_len_c-1 downto 0)=std_logic_vector(to_unsigned(APB_REG_WRITE_AVAIL,apb_addr_len_c))) else '0';
