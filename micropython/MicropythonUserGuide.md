# Introduction to dgrid MicroPython programming

dgrid provides a Python interface to construct a graph of execution and then to schedule for the execution of the Graph.

Graph performs many vision and AI tasks. The execution of the Graph is running with full dgrid acceleration mode so there is not much loss of performance when applications are running as python programs. 
...
dgrid python programming begins with the import of module dgrid
...
- color: color of the resulted output tensor
    ...
    - dgrid.MONO1: Single channel monochrome 

    - dgrid.MONO3: 3-channel monochrome where R,G,B channels have same value.

    - dgrid.COLOR: 3-channel RGB color.

- format: Format of the resulted output tensor 

    - dgrid.PLANAR: channels are seperated as RRR....GGG....BBB....
   
    - dgrid.INTERLEAVED: channels are seperated as RGBRGBRGB....
...
- input: Input tensor. This node expects input to be color=dgrid.MONO1 and format=dgrid.PLANAR.

- output: Output tensor. It has color=dgrid.MONO1 and format=dgrid.PLANAR  
...
- input: Input tensor. This node expects input to be color=dgrid.COLOR and format=dgrid.PLANAR.

- output: Output tensor. Output has color=dgrid.COLOR and format=dgrid.PLANAR  
...
- input: Input tensor. This node expects input to have color=dgrid.COLOR and format=dgrid.PLANAR

- output: Motion is produced as color-coded pixel. Output has color=dgrid.COLOR and format=dgrid.PLANAR 
...
- output: Output tensor has color=dgrid.COLOR and format=dgrid.PLANAR 
...
- input: Input tensor. This node expects input to have color=dgrid.COLOR and format=dgrid.PLANAR and image size=224x224
...
- input: Input tensor. This node expects input to have color=dgrid.COLOR and format=dgrid.PLANAR and image size=300x300
...
- dgrid.CanvasDrawText(text,row,col): Draw a string at location [row,col]

- dgrid.CanvasDrawPoint(r,c): Draw a point at location [row,col]

- dgrid.CanvasDrawRectangle([top_left_row,top_lef_col],[bot_right_row,bot_right_col]) : Draw a rectangle by specifying the topleft and botright corners.

- dgrid.DisplayFlushCanvas(): Flush the canvas to the display screen.
