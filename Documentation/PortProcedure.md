...
The example provided with this repo is meant to be reference design and it is implemented on the Arty-A7 development kit from Digilent and based on Xilinx Artix-7 FPGA. However dgrid, both SW and HW stack, can be ported to any FPGA/ASIC and SOC platform based on the procedure described below. 
...
- Update NUM_PCORE in [SW/base/dgrid.h](../SW/base/dgrid.h) to be 8 for large version and 4 for small version. This must be the same value as pid_gen_max_c configured in [HW/src/config.vhd](../HW/src/config.vhd) 
...
- Refer [here](../HW/riscv/README.md) if you like to use a different RISCV implementation. dgrid by default uses [DgsRiscv](https://github.com/SpinalHDL/DgsRiscv)
