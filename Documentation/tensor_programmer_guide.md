...
This is one of the key and unique capabilities of dgrid.
...
```
uint32_t func;
func=dgridmBuildKernelFunc($convolution::exe,np,nt); // Assign function pointer
> EXE_LOCKSTEP(func);
```
...
The stream processor lookup/interpolation table is constructed by host as described in [Application Programmer Guide](https://github.com/dgrid/dgrid/blob/master/Documentation/app_programmer_guide.md)
...
Thread switching is not automatic but manual by calling to dgridmTaskYield function below.
...
### dgridmTaskSpawn(void (*func)(void*,int),void *parm,int pid)
