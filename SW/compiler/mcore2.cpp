...
//----------------------------------------------------------------------------
// Copyright [2026] [Deepgrid Semi Pvt Ltd]c
//
...
void genEXE(FILE *out,uint32_t opcode,char end) {
   if(end==END_DONE) {
      ...
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%d;",opcode,REG_FPU_EXE,1);
      fprintf(out,"DGRIDM_GREG(0,%d,0)=(%d+(%d<<3));",REG_DP_RUN,DP_OPCODE_FPU_EXE,0);
   }
   else if(end==END_CONT)
      // More command to follow, this command must be completed before moving to next in the sequencce
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%d;",opcode,REG_FPU_EXE,0);
   else if(end==END_CONT_FAST)
      // More command to follow, dont have to wait for this command to be completed before moving to 
      // next one in the sequence
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%d;",opcode,REG_FPU_EXE,2);
   else {
      ...
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=(_end_==0)?1:((_end_=='.')?0:2);",opcode,REG_FPU_EXE);
      fprintf(out,"{if(_end_== 0)DGRIDM_GREG(0,%d,0)=(%d+(%d<<3));}",REG_DP_RUN,DP_OPCODE_FPU_EXE,0);
   }
}
...
static void genParm(FILE *out,uint32_t _attr,std::string &type,std::string &parm,bool isPointer) {
   ...
   if(!_isConstant) {
      if(_attrDynamic)
         fprintf(out,"DGRIDM_GREG(0x%x|(%s),0x%x,0)=%s;",_attr,type.c_str(),REG_FPU_SET,parm.c_str());
      else {
         if(isPointer)
            fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%s;",_attr,REG_FPU_SET,parm.c_str());
         else {
            if(strcasecmp(type.c_str(),"float")==0)
               fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=*((uint32_t *)&(%s));",_attr,REG_FPU_SET,parm.c_str());
            else
               fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%s;",_attr,REG_FPU_SET,parm.c_str());
         }
      }
      ...
      if(_isConstantFloat) {
         if(_attrDynamic)
            fprintf(out,"DGRIDM_GREG(0x%x|(%s),0x%x,0)=0x%x;",_attr,type.c_str(),REG_FPU_SET,*((uint32_t *)&_v));
         else
            fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0x%x;",_attr,REG_FPU_SET,*((uint32_t *)&_v));
      } else {
         if(_attrDynamic)
            fprintf(out,"DGRIDM_GREG(0x%x|(%s),0x%x,0)=%d;",_attr,type.c_str(),REG_FPU_SET,v2);
         else
            fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%d;",_attr,REG_FPU_SET,v2);
      }
   }
}
   ...
   char end) 
{
   ...
   for(i=0;i < (int)names.size();i++) {
      if(strcasecmp(names[i].c_str(),"N")==0) {
         fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=%s;",FPU_SET_P_CNT,REG_FPU_SET,parms[i].c_str());    
      } else {
   ...
   if(!c_valid && !x1_valid && !x2_valid)
      error(cMcore::M_currLine, "Missing parameter");
   fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_B,REG_FPU_SET);
   if(!a_valid)
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_C2,REG_FPU_SET);
   if(!x1_valid)
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0x3f800000;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_X,REG_FPU_SET);
   if(!x2_valid)
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0x3f800000;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_Y,REG_FPU_SET);
   if(!c_valid)
      fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0x3f800000;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_C,REG_FPU_SET);
   ...
   char end) 
{
   ...
   if(!a_valid && !group_valid) {
      ...
      if(abs)
         fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_C,REG_FPU_SET);
      else
         fprintf(out,"DGRIDM_GREG(0x%x,0x%x,0)=0xFF7FFFFF;",FPU_SET_W_FP32|FPU_SET_M_VALUE|FPU_SET_P_C,REG_FPU_SET);
   }
