...
// mcore programs are C-program with embedded extension for special dgrid tensor
// operations.
...
{
   fprintf(fp, "DGRIDM_GREG(%d,%d,%d)=%s;", p1, p2, p3, s);
}
...
char *cMcore::scan_exe(FILE *out, char *line, bool lockstep)
{
   ...
   if (specifier.size() == 1)
   {
      fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=(%s);",
         s_dgridmFifoReady, REG_DP_RUN, (char *)specifier[0].m_v.c_str());
      return line;
   }
   ...
   if (addr < 0)
   {
      ...
      fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=DP_EXE_CMD(%d,%s,%s,0,%s,%s,%s,%s);",
         s_dgridmFifoReady, REG_DP_RUN,
   ...
   else
   {
      ...
      fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=DP_EXE_CMD(%d,%d,%s,0,%d,%d,%s,%d);",
         s_dgridmFifoReady, REG_DP_RUN,
...
char *cMcore::scan_notify(FILE *out, char *line)
{
   ...
   fprintf(out, "DGRIDM_GREG(0,%d,0)=((uint32_t)((%s)));", REG_DP_INDICATION_PARM0, specifier[0].m_v.c_str());
   fprintf(out, "DGRIDM_GREG(0,%d,0)=(int)(%s);", REG_DP_INDICATION_PARM1, specifier[1].m_v.c_str());
   fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=(%d+(%d<<3));", s_dgridmFifoReady, REG_DP_RUN,
...
char *cMcore::scan_print(FILE *out, char *line)
{
   ...
   if (specifier.size() != 2)
      error(cMcore::M_currLine, "Invalid parameters");
   fprintf(out, "DGRIDM_GREG(0,%d,0)=((int)(%s));", REG_DP_INDICATION_PARM0, specifier[0].m_v.c_str());
   fprintf(out, "DGRIDM_GREG(0,%d,0)=(int)(%s);", REG_DP_INDICATION_PARM1, specifier[1].m_v.c_str());
   fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=(%d+(%d<<3));", s_dgridmFifoReady, REG_DP_RUN,
      DP_OPCODE_PRINT, 0);
   return line;
...
void cMcore::gen_global_assign(FILE *out, cMcoreTerm &left, cMcoreTerm &right)
{
   ...
   if (index < 0)
      error(cMcore::M_currLine, "Invalid global reference");
   fprintf(out, "DGRIDM_GREG(0,%d,0)=(%s);", REG_DP_INDICATION_PARM0 + index, right.m_specifier[0].m_v.c_str());
}
...
void cMcore::gen_transfer(FILE *out, cMcoreTerm &left, cMcoreTerm &right, char *flushCondition, std::vector<cMcoreSpecifier> *stream_id)
{
   ...
   fprintf(out, "%s;DGRIDM_GREG(0,%d,0)=DP_TRANSFER_CMD(%d,0,%d,%d,%d,%d,%d,%d,((%s)>=0)?1:0,(%s)&(%d),", s_dgridmFifoReady,
