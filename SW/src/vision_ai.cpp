...
//---------------------------------------------------------------------
// Example on how to use dgrid for vision AI applications
// Input from webcam are processed by dgrid
...
int vision_ai() {
   ...
   if(testcase==TestCaseEdgeDetection) {
      ...
      // Graph to show multitasking capability of dgrid
   ...
   while(1) {
      ...
      if(CameraCaptureReady()) {
         ...
         tensorInput.Alias((DGRID_SHARED_MEM)CameraGetCapture());
         if(testcase==TestCaseAll) {
            for(int i=0;i < MAX_OUTPUT;i++) {
               tensorOutputs[i].Alias((DGRID_SHARED_MEM)displayBuffer);
            }
         } else {
            tensorOutput.Alias((DGRID_SHARED_MEM)displayBuffer);
         }
