...
// This is the test suite for dgrid
...
int test_color()
{
   DGRID_SHARED_MEM input_share;
   DGRID_SHARED_MEM output_share;
   ...
   input_share=dgridAllocSharedMem(4*MAX_PICT_DIM*MAX_PICT_DIM);
   output_share=dgridAllocSharedMem(4*MAX_PICT_DIM*MAX_PICT_DIM);
   input=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(input_share);
   output=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(output_share);
   ...
   free(split_result);
   dgridFreeSharedMem(input_share);
   dgridFreeSharedMem(output_share);
   return 0;
}
...
//led();
   ...
   FILE *fp;
   size_t size;
   uint8_t *boxes=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(TF1.BufferGetInterleave(TF1.m_operators[TF1.m_operators.size()-1]->m_def.input[0]));
   uint8_t *classes=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(TF1.BufferGetInterleave(TF1.m_operators[TF1.m_operators.size()-1]->m_def.input[1]));
