...
// This file implements the API between micropython and dgrid
...
//
// Initialize dgrid from micropython
//
void MPY_Init() {
   dgridInit();
   GdiInit();
...
bool MPY_TENSOR_GetCameraCapture(MPY_HANDLE _tensor) {
   ...
   if(_tensor != 0) {
      ...
      if(tensor) 
         tensor->Alias((DGRID_SHARED_MEM)CameraGetCapture());
   }
   else {
...
bool MPY_TENSOR_GetScreenCanvas(MPY_HANDLE _tensor) {
   ...
   if(_tensor != 0) {
      tensor=(TENSOR *)_tensor;
      if(tensor)
         tensor->Alias((DGRID_SHARED_MEM)DisplayGetBuffer()); 
   }
   else {
