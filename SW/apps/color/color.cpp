...
DgridStatus GraphNodeColorAndReshape::Create(TENSOR *input,TENSOR *output,
                                          TensorObjType _dstColorSpace,
                                          TensorFormat _dstFormat,
                                          int clip_x,int clip_y,
                                          int clip_w,int clip_h,
...
DgridStatus GraphNodeColorAndReshape::Verify() {
   ...
   if(m_srcColorSpace==TensorObjTypeYUYV) {
      ...
      if(m_spu) 
         dgridFreeSharedMem(m_spu);
      m_spu=dgridBuildSpuBundle(1,SpuCallback,0,0,0);
...
DgridStatus GraphNodeColorAndReshape::Execute(int queue,int stepMode) {
   if(m_srcColorSpace==TensorObjTypeYUYV) {
      kernel_yuyv2rgb_exe(
         GetJobId(queue),
