...
DgridStatus GraphNodeEqualize::Create(TENSOR *input,TENSOR *output) {
   Cleanup();
   m_input=input;
   m_output=output;
...
DgridStatus GraphNodeEqualize::Verify() {
...
DgridStatus GraphNodeEqualize::Execute(int queue,int stepMode) {
   if(m_histogramAvail) {
      GenEqualizer();
   }
   kernel_equalize_exe(
...
DGRID_SHARED_MEM GraphNodeEqualize::GenEqualizer() {
   int16_t *histogram;
   int count;
   bool flag;
   int overflow;
   ...
   m_spu=dgridBuildSpuBundle(1,SpuCallback,this,0,0);
   return m_spu;
}
