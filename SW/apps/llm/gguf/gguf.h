...
public:
    ...
    DgridStatus SaveAsZUF(const char* modelName,bool forSim,ZUF_QUANT quant);
    DgridStatus Verify(const char* fname);
...
private:
    DgridStatus quantize(float* x, size_t sz, int N, int D,bool reorder,ZUF_QUANT quant);
private:
    uint8_t* m_top;
    size_t m_curr;
    size_t m_size;
