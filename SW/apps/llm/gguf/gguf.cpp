...
DgridStatus GGUF::quantize(float* x, size_t sz, int N, int D,bool reorder,ZUF_QUANT quant) {
...
DgridStatus GGUF::Open(const char* fname)
{
    FILE* fp = 0;
    uint8_t *magic;
    std::string key;
...
// dgrid works with ZUF format

DgridStatus GGUF::SaveAsZUF(const char* modelName,bool forSim,ZUF_QUANT quant) {
    size_t sz;
    uint8_t* tensor;
    uint32_t dim;
    uint32_t hidden_dim;
    ...
    if (zuf.Create(modelName) != DgridStatusOk)
        return DgridStatusFail;
