...
// Open the ZUF file
// ZUF file is the model file for dgrid
// Convert GGUF to ZUF by SW/gguf/quant

DgridStatus llama::Open(const char* checkpoint_path) {
    int kv_dim;
    int head_size;
    uint32_t sz;
...
DgridStatus llama::SetSamplingPolicy(float temperature,float p) {
    if(temperature==0)
        return DgridStatusFail;
    if(p < 0.0 || p > 1.0)
        return DgridStatusFail;
...
//
DgridStatus llama::SetSamplingPolicyGreedy() {
    m_samplingGreedy = true;
...
// Otherwise dgrid uses a combination of top-k top-p and temperature for sampling
...
DgridStatus llama::SystemPrompt(char *prompt) {
    char* piece;
    m_output = 0;
...
DgridStatus llama::UserPrompt(char *userPrompt,std::string *output) {
    int token=0;
    float16_t* logits;
    char* piece;
    int pos;
