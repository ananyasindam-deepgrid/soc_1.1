...
// are transfered from DDR to dgrid.
...
// *** Note *** that reference implementation does not have weights memory layout of dgrid so the 
...
// of dgrid so the implementation is very different but it shows the expected 
// functions.
//----------------------------------------------------------------------------
...
void kernel_llm_matmul_q4_exe(
   ...
   dgridDualHartExecute(matmul_q4,&req);
...
void kernel_llm_matmul_q8_exe(
   ...
   dgridDualHartExecute(matmul_q8,&req);
...
void kernel_llm_dot_product_exe(int reqId,int N,int K,float16_t *x1,float16_t *_x2,int _x2_dim,float16_t *_y,float scale)
{
   ...
   dgridDualHartExecute(llm_dot_product_exe,&req);
...
void kernel_llm_dot_product2_exe(int reqId,int N,int _K,float16_t *x1,float16_t *x2,int x2_dim,float16_t *_y)
{
   ...
   dgridDualHartExecute(llm_dot_product2_exe,&req);
...
void kernel_llm_rms_exe(int reqId,int N,float16_t *x,float16_t *o,float *w)
{
   ...
   dgridJobDone(123);
   for(;;) {
      if(dgridReadResponse(&resp) && resp==123)
         break;
   }
...
void kernel_llm_done()
{
   ...
   dgridJobDone(reqid); 
   for(;;) {
      if(dgridReadResponse(&resp) && resp==reqid)
         break;
   }
   reqid++;
}
...
void kernel_tick()
{
   ...
   dgridJobDone(98); 
   for(;;) {
      if(dgridReadResponse(&resp) && resp==98)
         break;
   }
   mytick = Time2Get();
}
