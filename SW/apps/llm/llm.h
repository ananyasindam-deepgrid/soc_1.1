...
public:
    llama();
    ~llama();
    DgridStatus Open(const char* checkpoint_path);
    DgridStatus SystemPrompt(char *prompt);
    DgridStatus UserPrompt(char *userPrompt,std::string *output);
    void Clear();
    void Close();
    DgridStatus SetSamplingPolicy(float temperature,float p);
    DgridStatus SetSamplingPolicyGreedy();
    void ClearStat();
