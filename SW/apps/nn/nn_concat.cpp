...
DgridStatus NeuralNetLayerConcat::Prepare() {
   ...
   for(uint32_t i=0;i < op->u.concat.num_input;i++) {
      DGRID_SHARED_MEM spu;
      spu=dgridBuildSpuBundle(1,SpuEval,this,i,0);
...
DgridStatus NeuralNetLayerConcat::Evaluate(int queue) {
   NeuralNetOperatorDef *op=&m_def;
   int destIndex=0;
   int idx=0;
   int cnt;
