...
DgridStatus TfliteNn::Create(const char *fname,TENSOR *_input,int numOutput,...) {
   va_list args;
   TENSOR *t;
   ...
   for (uint32_t subgraph_index = 0; subgraph_index < (uint32_t)subgraphs->size();
      ...
      for (uint32_t i = 0; i < (uint32_t)operators->size(); ++i) {
         ...
         if(!(layer=NeuralNet::CreateLayer(i,&def)))
            return DgridStatusFail;
         if(layer->Prepare() != DgridStatusOk)
            return DgridStatusFail;
      }
      NeuralNet::LoadEnd();
   }
