...
public:
   TfliteNn();
   virtual ~TfliteNn();
   DgridStatus Create(const char *fname,TENSOR *_input,int numOutput,...);   
   virtual DgridStatus Verify();
   DgridStatus Load(const char *fname,TENSOR *_input,int numOutput,...);
   DgridStatus Unload();
private:
   // Parsing functions
   ...
   DgridStatus PopulateConvolutionQuantizationParams(
   ...
   DgridStatus GetQuantizedConvolutionMultipler(const TfliteNnTensorDef* input,
   ...
   DgridStatus CalculateActivationRangeQuantized(NeuralNetActivation activation,
            TfliteNnTensorDef* output,
            int32_t* act_min,
            int32_t* act_max);
