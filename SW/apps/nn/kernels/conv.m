...
static void convolution_3x3(void *_p,int pid) {
   ...
   if(x > CONV_SMALL_BOT_DX) dgridAbort(0);
   if(y > CONV_SMALL_BOT_DY) dgridAbort(0);
   if(req->group > 2) dgridAbort(0);
...
static void convolution_1x1(void *_p,int pid) {
   ...
   for(group=NUM_PCORE,groupsz=1;group >= 1;group=(group>>1),groupsz=(groupsz<<1)) {
      ...
      f_activate=dgridBuildKernelFunc($convolution1x1::activate,np,conv_dx);
      ...
      for(i=from;i < to;i += step)
      {
         ...
         for(mm=0;mm < topsz;) {
            ...
            f=dgridBuildKernelFunc(kfunc[dycnt2-1],NUM_PCORE,conv_dx);
...
static void convolution_depthwise(void *_p,int pid) {
   ...
   for(group=NUM_PCORE;group >= mingroup;group=group/2) {
      ...
      if(x > CONV_DEPTHWISE_BOT_DX) dgridAbort(0);
      if(y > CONV_DEPTHWISE_BOT_DY) dgridAbort(0);
      ...
      f=dgridBuildKernelFunc($convolution_depthwise::exe3x3,np,(dxcnt==1)?conv_dx:NUM_THREAD_PER_CORE/2+conv_dx);
...
)
{
   ...
   dgridDualHartExecute(do_add_process,&req);

   dgridJobDone(_req_id);
}
...
)
{
   ...
   if(req.ksz==1)
   {
      dgridDualHartExecute(convolution_1x1,&req);
   }
   else
   {
      dgridDualHartExecute(convolution_3x3,&req);
   }
   dgridJobDone(_req_id);
}
...
)
{
   ...
   dgridDualHartExecute(convolution_depthwise,&req);
