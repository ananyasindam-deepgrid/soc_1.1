...
DgridStatus NeuralNetLayerConv2D::Prepare() {
   ...
   m_shmSpu=dgridBuildSpuBundle(3,
...
DgridStatus NeuralNetLayerConv2D::Evaluate(int queue) {
   NeuralNetOperatorDef *op=&m_def;
   int in_fmt,out_fmt;
...
DgridStatus NeuralNetLayerConv2D::ConvolutionStrategy(int topcnt, int topdim, int botcnt, int botdim,int ksz,int stride,int num_pcore,
...
DGRID_SHARED_MEM NeuralNetLayerConv2D::GenConvolutionWeight(uint8_t *_coef,std::vector<int> &shape,int topcnt,int botcnt,int kz) {
   ...
   out = (uint8_t *)DGRID_SHARED_MEM_VIRTUAL(shm);
...
void NeuralNetLayerConv2D::GenBias(int32_t *bias,int biasLen,int32_t activationBias,DGRID_SHARED_MEM *shmHi,DGRID_SHARED_MEM *shmLo) {
   DGRID_SHARED_MEM biasLo_p,biasHi_p;
   ...
   biasLo = (int16_t *)DGRID_SHARED_MEM_VIRTUAL(biasLo_p);
   biasHi = (int16_t *)DGRID_SHARED_MEM_VIRTUAL(biasHi_p);
...
DGRID_SHARED_MEM NeuralNetLayerConv2D::GenFcWeight(uint8_t *_coef,int _topcnt,int _botcnt,int *coef_dim,int *_nthread,int *_npcore) {
   ...
   _gen_coef = (int8_t *)DGRID_SHARED_MEM_VIRTUAL(out_p);
