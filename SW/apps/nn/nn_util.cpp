...
DGRID_SHARED_MEM NeuralNet::BufferGetFlat(int bufid) {
   if(bufid < (int)m_bufLst.size())
...
DGRID_SHARED_MEM NeuralNet::BufferGetInterleave(int bufid) {
   if(bufid < (int)m_bufLst.size())
...
DGRID_SHARED_MEM NeuralNet::BufferAllocate(size_t sz) {
   DGRID_SHARED_MEM shm;
   shm=dgridAllocSharedMem(sz);
...
void NeuralNet::BufferAllocate(DGRID_SHARED_MEM shm) {
   m_bufUnboundLst.push_back(shm);
}
...
DgridStatus NeuralNet::BufferAllocatePrepare(int bufid,NeuralNetTensorType type,size_t sz,bool flatFmt,bool interleaveFmt) {
   assert(bufid >= 0);
   switch(type) {
...
DgridStatus NeuralNet::BufferAllocateCommit() {
   int i,j;
#if 0
   for(i=0;i < (int)m_bufLst.size();i++) {
      if(m_bufLst[i].shmFlatPresent && !m_bufLst[i].shmFlat)
         m_bufLst[i].shmFlat=dgridAllocSharedMem(m_bufLst[i].sz);
      if(m_bufLst[i].shmInterleavePresent && !m_bufLst[i].shmInterleave)
         m_bufLst[i].shmInterleave=dgridAllocSharedMem(m_bufLst[i].sz);
   }
#endif
#if 1
   for(i=0;i < (int)m_bufLst.size();i++) {
...
DgridStatus NeuralNet::BufferAllocate(int bufid,TENSOR *_tensor) {
   assert(bufid >= 0);
   if(bufid < (int)m_bufLst.size()) {
      if(m_bufLst[bufid].sz==0)
      ...
      if(_tensor->GetFormat()==TensorFormatSplit) {
         if(!m_bufLst[bufid].shmFlat) {
            ...
            if(!m_bufLst[bufid].shmFlatIsRef && (m_bufLst[bufid].shmFlatReused < 0))
               dgridFreeSharedMem(m_bufLst[bufid].shmFlat);
            m_bufLst[bufid].shmFlat=_tensor->GetBufShm();
            m_bufLst[bufid].shmFlatIsRef=true;
            m_bufLst[bufid].shmFlatPresent=true;
         }
      ...
      if(_tensor->GetFormat()==TensorFormatInterleaved) {
         if(!m_bufLst[bufid].shmInterleave) {
            ...
            if(!m_bufLst[bufid].shmInterleaveIsRef && (m_bufLst[bufid].shmInterleaveReused < 0))
               dgridFreeSharedMem(m_bufLst[bufid].shmInterleave);
            m_bufLst[bufid].shmInterleave=_tensor->GetBufShm();
            m_bufLst[bufid].shmInterleaveIsRef=true;
            m_bufLst[bufid].shmInterleavePresent=true;
         }
...
// Load label file 
DgridStatus NeuralNet::LabelLoad(const char *fname) {
   FILE *fp;
   char str[200];
   fp=fopen(fname,"r");
   if(!fp) {
...
DgridStatus NeuralNet::GetTop5(uint8_t *prediction,int predictionSize,int *top5)
{
   unsigned int v;
   unsigned int t;
   unsigned int i0;
