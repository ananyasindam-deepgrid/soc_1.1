...
NeuralNetLayerObjDetect::~NeuralNetLayerObjDetect() {
   if(m_anchors)
      free(m_anchors);
   if(m_scoreResult)
      dgridFreeSharedMem(m_scoreResult);
   if(m_classResult)
      dgridFreeSharedMem(m_classResult);
}

DgridStatus NeuralNetLayerObjDetect::Prepare() {
   ...
   m_scoreResult=dgridAllocSharedMem(sizeof(uint8_t)*numBoxes+8);
   m_classResult=dgridAllocSharedMem(sizeof(uint8_t)*numBoxes+8);
   return DgridStatusOk;
}

DgridStatus NeuralNetLayerObjDetect::Evaluate(int queue) {
   NeuralNetOperatorDef *op=&m_def;
   int i,j;
...
//return DgridStatusOk;
   ...
   boxes=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(op->input[0]));
   classes=(uint8_t *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(op->input[1]));
   ...
   float *box_p=(float *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(m_def.output[0]));
   float *classes_p=(float *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(m_def.output[1]));
   float *probabilty_p=(float *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(m_def.output[2]));
   float *numDetect_p=(float *)DGRID_SHARED_MEM_VIRTUAL(m_nn->BufferGetInterleave(m_def.output[3]));
   for(int i=0;i < numDetects;i++) {
