...
struct NeuralNetBuffer {
   DGRID_SHARED_MEM shmFlat;
   bool shmFlatIsRef;
   int shmFlatReused;
   int shmFlatLastUsed;
   int shmFlatFirstUsed;
   bool shmFlatPresent;
   DGRID_SHARED_MEM shmInterleave;
...
public:
   ...
   virtual DgridStatus Prepare()=0;
   virtual DgridStatus Evaluate(int queue)=0;
...
public:
   NeuralNet();
   virtual ~NeuralNet();
   DgridStatus LoadBegin(TENSOR *input,std::vector<TENSOR *> &_output);
   DgridStatus LoadEnd();
   virtual DgridStatus Unload();
...
public:
   ...
   static DgridStatus GetTop5(uint8_t *prediction,int predictionSize,int *top5);
   DgridStatus LabelLoad(const char *fname);
   const char *LabelGet(int _idx);
public:
   DGRID_SHARED_MEM BufferAllocate(size_t sz);
   void BufferAllocate(DGRID_SHARED_MEM shm);
   DgridStatus BufferAllocatePrepare(int bufid,NeuralNetTensorType type,size_t sz,bool flatFmt,bool interleaveFmt);
   DgridStatus BufferAllocateCommit();
   DgridStatus BufferAllocate(int bufid,TENSOR *_tensor);
   DGRID_SHARED_MEM BufferGetFlat(int bufid);
   DGRID_SHARED_MEM BufferGetInterleave(int bufid);
   bool BufferFlatPresent(int bufid);
   bool BufferInterleavePresent(int bufid);
   bool BufferIsInit(int bufid);
   void BufferFreeAll();
private:
   DgridStatus AssignInputTensor(bool firstTime);
   DgridStatus AssignOutputTensors(bool firstTime);
public:
...
private:
   ...
   std::vector<DGRID_SHARED_MEM> m_bufUnboundLst;
   int m_runningStep;
